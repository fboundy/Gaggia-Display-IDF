...
lv_obj_t *Temp_gauge;
lv_meter_indicator_t *current_temp_indic;
lv_meter_indicator_t *set_temp_indic;
lv_obj_t *FlashSize;
...
static void Onboard_create(lv_obj_t *parent) {
  ...
  Temp_gauge = lv_meter_create(panel1);
  lv_obj_set_size(Temp_gauge, 120, 120);
  lv_meter_scale_t *scale = lv_meter_add_scale(Temp_gauge);
  lv_meter_set_scale_range(scale, 60, 110, 270, 135);
  current_temp_indic = lv_meter_add_needle_line(
      Temp_gauge, scale, 4, lv_palette_main(LV_PALETTE_BLUE), -10);
  set_temp_indic = lv_meter_add_needle_line(
      Temp_gauge, scale, 4, lv_palette_main(LV_PALETTE_RED), -10);
  ...
  lv_obj_set_grid_cell(Temp_label, LV_GRID_ALIGN_START, 1, 1,
                       LV_GRID_ALIGN_START, 2, 1);
  lv_obj_set_grid_cell(Temp_gauge, LV_GRID_ALIGN_CENTER, 1, 1,
...
void example1_increase_lvgl_tick(lv_timer_t *t) {
  ...
  lv_meter_set_indicator_value(Temp_gauge, current_temp_indic,
                               MQTT_GetCurrentTemp());
  lv_meter_set_indicator_value(Temp_gauge, set_temp_indic, MQTT_GetSetTemp());
